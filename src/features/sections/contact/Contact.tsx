...
export default function Contact() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(sectionRef.current, { opacity: 0 }, { opacity: 1, duration: 0.8, scrollTrigger: { trigger: sectionRef.current, start: 'top 70%' } })
    }, sectionRef)
    return () => ctx.revert()
  }, [])
  ...
  return (
    <section ref={sectionRef} id='contact' className='opacity-0' style={{ padding: 'clamp(5rem,10vw,8rem) clamp(1.5rem,5vw,5rem)' }}>
      <div className='max-w-7xl mx-auto'>
        ...
        <div className='grid md:grid-cols-5 gap-14 md:gap-20'>
          ...
          <form onSubmit={handleSubmit} className='md:col-span-3 flex flex-col gap-5 p-8 rounded-[var(--r-xl)] border' style={{ background: 'var(--bg-card)', borderColor: 'var(--border-soft)', boxShadow: 'var(--shadow-sm)' }}>
            <div className='grid md:grid-cols-2 gap-5'>
              <div><label className={labelClass} style={{ color: 'var(--fg-muted)' }}>{t.form_name}</label>
                <input type='text' placeholder={t.form_name_placeholder} className={inputClass} style={{ borderColor: 'var(--border)', color: 'var(--fg)' }}
                  required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
              <div><label className={labelClass} style={{ color: 'var(--fg-muted)' }}>{t.form_email}</label>
                <input type='email' placeholder={t.form_email_placeholder} className={inputClass} style={{ borderColor: 'var(--border)', color: 'var(--fg)' }}
                  required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} /></div>
            </div>
            <div className='grid md:grid-cols-2 gap-5'>
              <div><label className={labelClass} style={{ color: 'var(--fg-muted)' }}>{t.form_phone}</label>
                <input type='tel' placeholder={t.form_phone_placeholder} className={inputClass} style={{ borderColor: 'var(--border)', color: 'var(--fg)' }}
                  value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} /></div>
              <div><label className={labelClass} style={{ color: 'var(--fg-muted)' }}>{t.form_service}</label>
                <select className={inputClass} style={{ borderColor: 'var(--border)', color: 'var(--fg)' }} value={form.service} onChange={e => setForm(f => ({ ...f, service: e.target.value }))}>
                  <option value=''>{t.form_service_placeholder}</option>
                  <option>{t.form_service_web}</option><option>{t.form_service_landing}</option>
                  <option>{t.form_service_ecommerce}</option><option>{t.form_service_app}</option><option>{t.form_service_other}</option>
                </select></div>
            </div>
            <div><label className={labelClass} style={{ color: 'var(--fg-muted)' }}>{t.form_message}</label>
