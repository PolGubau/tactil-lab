    ...
    <section ref={sectionRef} id='pricing' style={{ padding: 'clamp(5rem,10vw,8rem) clamp(1.5rem,5vw,5rem)' }}>
      <div className='max-w-7xl mx-auto'>
        <div className='pricing-header text-center mb-16 opacity-0'>
          ...
          <h2 className='text-4xl md:text-6xl font-black mb-4 tracking-tight text-balance'>{t.pricing_headline}</h2>
          <p className='max-w-lg mx-auto text-base text-muted text-pretty'>{t.pricing_subtitle}</p>
        </div>

        <div className='grid grid-cols-1 md:grid-cols-3 gap-5'>
          {plans.map((plan) => (
            <div key={plan.id} className='pricing-card opacity-0 relative flex flex-col p-8 transition-all duration-350'
              style={{
              ...
              <div className='mb-7 pb-7 border-b' style={{ borderColor: plan.isPopular ? 'rgba(247,244,239,0.12)' : 'var(--border-soft)' }}>
                <h3 className='text-xl font-black mb-2 text-balance'>{plan.title}</h3>
                <p className='text-sm leading-relaxed mb-5 text-pretty' style={{ color: plan.isPopular ? 'rgba(247,244,239,0.6)' : 'var(--fg-muted)' }}>{plan.desc}</p>
                {plan.price ? (
                  <div>
                    {plan.oldPrice && <span className='text-xs line-through tabular-nums' style={{ color: plan.isPopular ? 'rgba(247,244,239,0.4)' : 'var(--fg-subtle)' }}>{plan.oldPrice} EUR</span>}
                  ...
                  <div className='text-3xl md:text-4xl font-black text-accent'>Custom</div>
                )}
              </div>
